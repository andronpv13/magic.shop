...
| [06 — Оформление заказа и оплата] | Оформление заказа (checkout.php): адрес доставки, комментарий, создание заказа (транзакция). Страница оплаты (pay.php): выбор способа оплаты (демо-режим), статусы заказов |
...
├── checkout.php              # Оформление заказа (адрес доставки, комментарий). Создание заказа. Перенаправление на pay.php для выбора оплаты
├── pay.php                   # Оплата (СБП, банковская карта, наличные - демо-режим). Выбор способа оплаты после создания заказа
│
...
└── images/                   # Каталог изображений
    ...
    ├── logo_.png             # Основной логотип в header.php
    ├── main.png              # Главное изображение
    ├── main8x1.png           # Баннер 8x1 (латинская x)
    ...
    └── background/           # Каталог для загружаемых пользователем фонов (дефолтные файлы не требуются)

└── logs/                     # Логи приложения (директория создаётся автоматически при первом запросе)
...
| [checkout.php] | Оформление заказа (требует авторизации): адрес доставки, комментарий. Создание заказа с последующим перенаправлением на pay.php |
| [pay.php] | Страница выбора способа оплаты (демо-режим): СБП, карта, наличные. Вызывается после создания заказа в checkout.php |
...
| [images/logo_.png] | Основной логотип в header.php |
| [images/main.png] | Главное изображение для главной страницы |
| [images/main8x1.png] | Баннер 8:1 для главной страницы (латинская x) |
